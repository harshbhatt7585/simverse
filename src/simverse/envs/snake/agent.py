...
DEFAULT_AGENT_ACTIONS = np.arange(4, dtype=np.int64)


class SnakeAgent(SimAgent):
    def __init__(
        self,
        agent_id: int,
        ...
        super().__init__(
            name=name or f"snake_agent_{agent_id}",
    ...
    def info(self) -> dict:
        return {"agent_id": self.agent_id, "score": self.score, "reward": self.reward}
    ...
    def current_state(self) -> np.ndarray:
        return np.array([self.score, self.reward], dtype=np.float32)
